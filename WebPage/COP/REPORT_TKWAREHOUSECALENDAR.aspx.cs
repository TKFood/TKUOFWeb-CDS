...
public partial class CDS_WebPage_COP_REPORT_TKWAREHOUSECALENDAR : Ede.Uof.Utility.Page.BasePage
{
    ...
    private void BindGrid(string SALESFOCUS)
    {
        ...
        {
            QUERYS.AppendFormat(@" AND CONVERT(nvarchar,[EVENTDATE],112)  LIKE '{0}%'", RadDatePicker1.SelectedDate.Value.ToString("yyyyMM"));
        }


        cmdTxt.AppendFormat(@"

                             SELECT  [ID],CONVERT(nvarchar,[EVENTDATE],112) [EVENTDATE],[CAR],[EVENT]
                             FROM [TKWAREHOUSE].[dbo].[CALENDAR]
                             WHERE 1=1       
                             {0}
                             ORDER BY CONVERT(nvarchar,[EVENTDATE],112),CAR     
                                ", QUERYS.ToString());
