...
public partial class CDS_WebPage_SIGN_SIGNPURTATB : Ede.Uof.Utility.Page.BasePage
{
    ...
    private void BindGrid(string SDATE)
    {
        ...
        string cmdTxt = @" 
                       SELECT MQ002,TA001,TA002
                        ...
                        FROM [TK].dbo.PURTA,[TK].dbo.CMSMQ
                        WHERE TA001=MQ001
                        AND TA007='N' 
                        AND TA003 LIKE @SDATE+'%'
    ...
    public void UPDATEPURTAPURTB(string SQL)
    {
        string TA007 = "Y";
        string TA014= "160115";
        string TA016 = "N";
        ...
        try
        {
            using (SqlConnection cnn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand(SQLCOMMAND, cnn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@TA007", TA007);
                    cmd.Parameters.AddWithValue("@TA014", TA014);
                    cmd.Parameters.AddWithValue("@TA016", TA016);
                    cmd.Parameters.AddWithValue("@TB025", TB025);
    ...
    public string SETSQL(string TA001TA002)
    {
        StringBuilder SQL = new StringBuilder();

        SQL.AppendFormat(@"
                        UPDATE [TK].dbo.PURTA
                        SET TA007=@TA007,TA014=@TA014,TA016=@TA016, FLAG=FLAG+1,COMPANY=@COMPANY,MODIFIER=@MODIFIER ,MODI_DATE=@MODI_DATE, MODI_TIME=@MODI_TIME 
                        WHERE TA001+TA002 IN ({0})

                        UPDATE [TK].dbo.PURTB 
                        SET TB025=@TB025, FLAG=FLAG+1,COMPANY=@COMPANY,MODIFIER=@MODIFIER ,MODI_DATE=@MODI_DATE, MODI_TIME=@MODI_TIME 
                        WHERE TB001+TB002 IN ({0})
                        ", TA001TA002);
    ...
    protected void btn2_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow gvr in this.Grid1.Rows)
        {
            ...
            if (ck.Checked)
            {
                ...
                TA001TA002 += "'"+cell[2].Text+ cell[3].Text + "',";
            }
        }
        TA001TA002 += "''";
        Label3.Text = TA001TA002.ToString();

        string SQL = SETSQL(Label3.Text);

        UPDATEPURTAPURTB(SQL);
