...
<script>
    ...
    function loadSourceData() {
        ...
        $.each(data, function (i, item) {
            var obj = { "no": item.no, "name": item.name, "spec": item.spec, "deptqty": item.deptqty, "qty": item.qty, "deptname": item.deptname };
            itemsarr.push(obj);
        ...
        $("#Dept").append($("<option></option>").attr("value", deptname).text(deptname)).prop("disabled", true);
        $("#<%=hdContent.ClientID %>").val(JSON.stringify(itemsarr));
    }
        ...
        $.each(response, function (i, data) {
            if (i == 'data') {
                $.each(data, function (j, item) {
                    ...
                    var obj = { "no": item.ITEM_NO, "name": item.ITEM_NAME, "spec": item.SPECIFICATION, "deptqty": item.QUANTITY, "qty": qty, "deptname": deptname };
                    itemsarr.push(obj);
                    num++;
                });
                $("#<%=hdContent.ClientID %>").val(JSON.stringify(itemsarr));
