<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ItemPersontoPersonStockUC.ascx.cs" Inherits="WKF_OptionalFields_ItemPersontoPersonStockUC" %>
...
<div class="row" style="width:80%" id="divEdit" runat="server">
    ...
    <div class="col-lg-3 col-md-3 col-6"><select id="Dept"></select><div id="divDept"></div></div>

    <div class="col-lg-3 col-md-3 col-6">轉移人員</div>
    <div class="col-lg-3 col-md-3 col-6"><select id="DeptPerson"></select><div id="divDeptPerson"></div></div>
    
    <div class="col-lg-3 col-md-3 col-6">接收人員</div>
    <div class="col-lg-3 col-md-3 col-6"><select id="DeptPersonToPerson"></select><div id="divDeptPersonToPerson"></div></div>
...
<script>
    ...
    //var transferscrap = $("#ctl00_ContentPlaceHolder1_VersionFieldCollectionUsingUC_versionFieldUC3_DropDownList1").val();
    var bl = 0;
    var b2 = 0;
    var personname = "";//發放人員
    var receiver = "";//接收人員
    ...
    $("#DeptPerson").change(function ()
    {
        ...
        if (personname != $("#DeptPerson").val())
        {
            ...
            ajax_load_json('/RMTEST/Item/GetDeptPersonToPerson?deptperson=' + deptperson, null, loadDeptPersonToPersonCallback);
        }
        personname = $("#DeptPerson").val();
    });

    $("#DeptPersonToPerson").change(function () {
    ...
    function loadPersonSourceData() {
        ...
        $.each(data, function (i, item) {
            var obj = { "no": item.no, "name": item.name, "spec": item.spec, "deptqty": item.deptqty, "qty": item.qty, "deptname": item.deptname, "sender": item.sender, "receiver": item.receiver };
            itemsarr.push(obj);
            ...
            if (status == "APPLY") {
                ...
                $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'>" +
                    ...
                    "<td id='TD_receiver' style='display: none'>" + receiver + "</td>" +
                    "</tr>");
            }
            else {
                $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'>" +
        ...
        $("#DeptPersonToPerson").append($("<option></option>").attr("value", receiver).text(receiver)).prop("disabled", true).hide();
        //$("#Dept").hide();
        //$("#DeptPerson").hide();
        //$("#DeptPersonToPerson").hide();
        $("#divDept").text(deptname);
        $("#divDeptPerson").text(personname);
        $("#divDeptPersonToPerson").text(receiver);
    ...
    function loadDeptPersonCallback(response) {
        $.each(response, function (i, data) {
            $('#DeptPerson').empty();
            $('#DeptPersonToPerson').empty();
            $('#DeptPersonToPerson').append($('<option>', {
                value: 0,
                text: "請選擇人員"
            }));
            if (i == 'data') {
    ...
    //載入所有部門接收人員 combox
    function loadDeptPersonToPersonCallback(response) {
        $('#DeptPersonToPerson').empty();
        $.each(response, function (i, data) {
            if (i == 'data') {
                $('#DeptPersonToPerson').append($('<option>', {
    ...
    function getStockByDeptPersonCallback(response) {
        ...
        receiver = $("#DeptPersonToPerson").val();//接收人員
        $.each(response, function (i, data) {
            if (i == 'data') {
                $.each(data, function (j, item) {
                    var no = item.ITEM_NO;
                    ...
                    var obj = { "no": no, "name": name, "spec": spec, "deptqty": deptqty, "qty": qty, "deptname": deptname, "sender": personname, "receiver": receiver};
                    itemsarr.push(obj);
                    num++;
                });
    ...
    function tdchgSelect(obj) {
        ...
        itemsarr.forEach(function (item) {           
            if ($("#DeptPersonToPerson").val() == null) {
                item.receiver = $("#divDeptPersonToPerson").text().trim();
            } else {
                item.receiver = $("#DeptPersonToPerson").val().trim();
            }
