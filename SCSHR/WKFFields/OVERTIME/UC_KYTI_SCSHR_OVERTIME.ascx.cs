...
* 修改位置： 
    ...
    *   「btnCal_Click() -> BOImport -> 傳入時間(RESTMINS)」中，改為"hidRESTMINS_Min.Value"
...
* 修改人員：梁夢慈
* 修改項目：
    * 1.原取config參數(FORM_DEFAULT_START_TIME) -> 改為:OVERTIME_DEFAULT_START_TIME 
    *   原取config參數(FORM_DEFAULT_END_TIME) -> 改為:OVERTIME_DEFAULT_END_TIME 
* 修改原因：
    * 1.新增規格
* 修改位置： 
    * 1.「後端所有程式」中，SCSHRConfiguration.FORM_DEFAULT_START_TIME -> 取代為:SCSHRConfiguration.OVERTIME_DEFAULT_START_TIME
    *                     SCSHRConfiguration.FORM_DEFAULT_END_TIME -> 取代為:SCSHRConfiguration.OVERTIME_DEFAULT_END_TIME
* **/
...
* 修改人員：梁夢慈
* 修改項目：
    * 1.新增前端驗證送單卡控，起迄時間變更時檢查時間範圍日否正確("起"必須小於"迄")，不正確顯示「迄止時間小於起始時間」
* 修改原因：
    * 1.新增規格，因應客戶新需求，於2021/09/27已新增此功能顯示錯誤訊息文字，本次為加上送單卡控功能
* 修改位置： 
    * 1.「前端網頁、CheckTimeRange()」中，新增隱藏欄位(hidIsOverTimeRange)紀錄檢查時間範圍是否正確(0:正確 1:不正確)
    *   「前端網頁 -> checkVal()」中，新增判斷式檢查隱藏欄位(hidIsOverTimeRange)是否為不正確範圍，成立則擋單
* **/
...
* 修改原因：
    * 1.新增規格，因應客戶新需求
* 修改位置： 
    * 1.「SetField() -> // 起單或退回申請者、kcbIsPre_CheckedChanged()、
    *     kdtpOT_START_TextChanged()、kdtpOT_END_TextChanged()、
    *     kdtpPreOT_START_TextChanged()、kdtpPreOT_END_TextChanged」中，新增三元運算子判斷方法「CheckTimeRange()」，如成立就回寫訊息"迄止時間小於起始時間"
* **/
...
* 修改人員：梁夢慈
* 修改項目：
    * 1.檢查按鈕(btnChk)，文字左方新增紅色*
* 修改原因：
    * 1.新增規格，因應客戶新需求
* 修改位置： 
    * 1.「前端網頁 -> CSS語法」中，新增語法「span[id*='btnCal'] > .rbText:before」，創造偽元素
* **/
...
* 修改人員：梁夢慈
* 修改項目：
    * 1.新增表頭欄位-休息時間(kddlRESTMINS)，預設值「0, 0.5, 1, 1.5, 2 」小時
...
* 修改位置：
    * 1.「前端網頁中」中，新增欄位-休息時間(kddlRESTMINS)
...
* 修改位置：
    ...
    * *******
    * 「SetField」中，當現在是〈起單狀態〉，從〈SCSHRConfiguration.OVERTIME_DEFAULT_START_TIME〉和〈SCSHRConfiguration.OVERTIME_DEFAULT_END_TIME〉取得預設時間，並填入起訖時間
* **/
...
public partial class WKF_OptionalFields_UC_KYTI_SCSHR_OVERTIME : WKF_FormManagement_VersionFieldUserControl_VersionFieldUC
{
            ...
            if (!Page.IsPostBack) // 網頁首次載入
            {
                ...
                if (is_show_pre)
                {
                    if (string.IsNullOrEmpty(my_FieldValue))
                    {
                        ...
                        DateTime.TryParse(string.Format(@"{0} {1}", System.DateTime.Now.Date.ToString("yyyy/MM/dd"), SCSHR.SCSHRConfiguration.OVERTIME_DEFAULT_START_TIME), out dtDefaultStart);
                        ...
                        DateTime.TryParse(string.Format(@"{0} {1}", System.DateTime.Now.Date.ToString("yyyy/MM/dd"), SCSHR.SCSHRConfiguration.OVERTIME_DEFAULT_END_TIME), out dtDefaultEnd);
                ...
                switch (fieldOptional.FieldMode) // 判斷FieldMode
                {
                    ...
                    case FieldMode.ReturnApplicant:
                             ...
                             string.IsNullOrEmpty(fieldOptional.FieldValue)) // 起單時專用操作狀態或初始值
                        {
                            ...
                            ktxtAPPLICANT.Text = KUser.NameWithAccount; // 申請人姓名
                        ...
                        ktxtApplyHoursThemon.Text = getOverTimeHours(hidAPPLICANT.Value,
                               ...
                               true).ToString();
                        lblMessage.Text = CheckTimeRange() ? "迄止時間小於起始時間" : "";
    ...
    protected void btnCal_Click(object sender, EventArgs e)
    {
            ...
            !string.IsNullOrEmpty(kdtpOT_START.Text))
        {
                ...
                dtCalcResult.Rows.Count > 0)
            {
                ...
                decimal.TryParse(dtCalcResult.Rows[0]["OverTimeHours"].ToString(), out OverTimeHours);
                //REST_Min = REST_Hr * 60;
                ...
                ktxtOverTimeHours.Text = (OverTimeHours - REST_Hr).ToString(); // 表單顯示加班時數=飛騰加班時數-休息時數
            ...
            {
                hidAPIResult.Value = "ERROR";
                lblMessage.Text = "檢查失敗，無加班類別";
                DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"UC_KYTI_SCSHR_OVERTIME.btnCal_Click::無加班類別"));
            }
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void kdtpPreOT_END_TextChanged(object sender, EventArgs e)
    {
        lblMessage.Text = CheckTimeRange() ? "迄止時間小於起始時間" : "";
    }
    ...
    protected void btnAPPLICANT_DialogReturn(object sender, string result)
    {
        ...
        if (jArray.Count > 0)
        {
            ...
            KYT_EBUser KUser = new KYT_UserPO().GetUserDetailByUserGuid(this.ApplicantGuid); // 人員
            ktxtAPPLICANT.Text = KUser.NameWithAccount; // 申請人名稱
    ...
    {
        bool IsAlert = false;
        if (kcbIsPre.Checked) // 檢查預加班時間起迄
        {
            if (kdtpPreOT_START.SelectedDate != null && kdtpPreOT_END.SelectedDate != null)
            {
                if (DateTime.Compare((DateTime)kdtpPreOT_START.SelectedDate, (DateTime)kdtpPreOT_END.SelectedDate) > 0)
                {
                    IsAlert = true;
                }
            }
        ...
        {
            if (kdtpOT_START.SelectedDate != null && kdtpOT_END.SelectedDate != null)
            {
                if (DateTime.Compare((DateTime)kdtpOT_START.SelectedDate, (DateTime)kdtpOT_END.SelectedDate) > 0)
                {
                    IsAlert = true;
                }
            }
        }
        hidIsOverTimeRange.Value = IsAlert ? "1" : "0";
        return IsAlert;
    }
