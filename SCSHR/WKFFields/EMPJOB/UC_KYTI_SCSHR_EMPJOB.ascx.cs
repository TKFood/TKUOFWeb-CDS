...
* 修改位置：
    * 「SetField」中，當〈!Page.IsPostBack〉時，〈kdpSTARTDATE〉、〈kdpTRIALDATE〉、〈kdpSSTARTDATE〉、〈kdpGARRIVEDATE〉的屬性〈TextBoxReadOnly〉由〈SCSHRConfiguration.IS_PICKER_READONLY〉控制
* **/
...
/// 飛騰到職資訊
/// </summary>
public partial class WKF_OptionalFields_UC_KYTI_SCSHR_EMPJOB : WKF_FormManagement_VersionFieldUserControl_VersionFieldUC
{
    /// <summary>
    /// 資料庫連通字串
    /// </summary>
    ...
    public override string ConditionValue
    {
        get
        {
            ...
            // if (Page.Request.Browser.Browser == "InternetExplorer" || Page.Request.Browser.Browser == "IE" || Page.Request.Browser.Browser == "Mozilla")

            string cv = new JGlobalLibs.UOFUtils.ConditionValue()
            .Add("dept", ktxtAPPLICANTDEPT.Text)
            .ToString();
            DebugLog.Log(DebugLog.LogLevel.Info, string.Format(@"UC_KYTI_SCSHR_EMPJOB.RealValue.cv:{0}", cv));
            DebugLog.Log(DebugLog.LogLevel.Info, string.Format(@"Browser:{0}", Page.Request.Browser.Browser));
    ...
    public override string DisplayTitle
    {
        get
        {
            //表單查詢或WebPart顯示的標題
            //回傳字串
            return string.Format(@"{0}, 異動性質:{1}",
                ktxtEMPLOYEE.Text,
                kddlADJUSTTYPE.SelectedItem.Text);
        }
    }
    ...
    public override string RealValue
    {
        get
        {
            ...
            //     .ToString();
            // DebugLog.Log(DebugLog.LogLevel.Info, string.Format(@"UC_KYTI_SCSHR_EMPJOB.RealValue.rv:{0}", rv));
            // return rv;
            return String.Empty;
        }
        set
