...
* 修改位置：
    * 「SetField」中，當〈!Page.IsPostBack〉時，〈kdpRESIGNDATE〉和〈kdpRESIGNATIONDATE〉的屬性〈TextBoxReadOnly〉由〈SCSHRConfiguration.IS_PICKER_READONLY〉控制
* **/
...
* 修改項目：
    *  起單關沒有填寫的欄位，WS卻是必填
* 修正位置：
    * 「kdpRESIGNDATE_TextChanged」將〈預定離職日〉的值填入〈實際離職日〉
* **/

/// <summary>
/// 飛騰離職資訊
/// </summary>
public partial class WKF_OptionalFields_UC_KYTI_SCSHR_RESIGN : WKF_FormManagement_VersionFieldUserControl_VersionFieldUC
{
    /// <summary>
    /// 資料庫連通字串
    /// </summary>
    ...
    public override string ConditionValue
    {
        get
        {
            ...
            string cv = new JGlobalLibs.UOFUtils.ConditionValue()
                ...
                .ToString();
            DebugLog.Log(DebugLog.LogLevel.Info, string.Format(@"UC_KYTI_SCSHR_RESIGNVE.RealValue.cv:{0}", cv));
    ...
    public override string DisplayTitle
    {
        get
        {
            //表單查詢或WebPart顯示的標題
            //回傳字串
            return string.Format(@"{0}, 預計離職日:{1}",
                ktxtRESIGNEMP.Text,
                kdpRESIGNDATE.Text);
        }
    }
    ...
    public override string RealValue
    {
        get
        {
            ...
            //     .ToString();
            // DebugLog.Log(DebugLog.LogLevel.Info, string.Format(@"UC_KYTI_SCSHR_RESIGNVE.RealValue.rv:{0}", rv));
            // return rv;
            return String.Empty;
        }
        set
            ...
            if (!Page.IsPostBack) // 網頁首次載入
            {
                ...
                switch (fieldOptional.FieldMode) // 判斷FieldMode
                {
                    ...
                    case FieldMode.ReturnApplicant:
                        ...
                        kdpRESIGNATIONDATE.ViewType = KYTViewType.ReadOnly; // 實際離職日
                        ktxtMANAGERTALK.ViewType = KYTViewType.ReadOnly; // 主管面談
                        ktxtHRTALK.ViewType = KYTViewType.ReadOnly; // HR面談
                        chkINRESIGNRATE.ViewType = KYTViewType.ReadOnly; // 納入離職率計算
                        chkNEVERHIRED.ViewType = KYTViewType.ReadOnly; // 永不錄用
                        chkALLOWREHIRED.ViewType = KYTViewType.ReadOnly; // 優先再聘
                        ktxtNEVERHIREDREASON.ViewType = KYTViewType.ReadOnly; // 永不錄用原因

                        if (this.FormFieldMode == FieldMode.Applicant)
                        {
                            //EBUser KUser = new UserUCO().GetEBUser(this.ApplicantGuid);
                            ...
                            hidRESIGNEMPTitleId.Value = KUser.Title_ID[0];
                            hidRESIGNEMPTitleName.Value = KUser.Title_Name[0];
                        ...
                        // 設定Picker是否能輸入
                        kdpRESIGNATIONDATE.TextBoxReadOnly = SCSHRConfiguration.IS_PICKER_READONLY.ToUpper() == "Y";
                        kdpRESIGNDATE.TextBoxReadOnly = SCSHRConfiguration.IS_PICKER_READONLY.ToUpper() == "Y";
                        break;
                    ...
                    case FieldMode.Signin: // 表單簽核
                        kdpRESIGNATIONDATE.ViewType = KYTViewType.Input; // 實際離職日
                        ktxtMANAGERTALK.ViewType = KYTViewType.Input; // 主管面談
                        ktxtHRTALK.ViewType = KYTViewType.Input; // HR面談
                        chkINRESIGNRATE.ViewType = KYTViewType.Input; // 納入離職率計算
                        chkNEVERHIRED.ViewType = KYTViewType.Input; // 永不錄用
                        chkALLOWREHIRED.ViewType = KYTViewType.Input; // 優先再聘
                        ktxtNEVERHIREDREASON.ViewType = KYTViewType.Input; // 永不錄用原因
                        break;
                ...
                switch (fieldOptional.FieldMode) // 判斷FieldMode
                {
                    ...
                    case FieldMode.Signin: // 表單簽核
                        ...
                        if (fieldOptional.FieldMode != FieldMode.Signin)
                            kdpRESIGNDATE.ViewType = KYTViewType.Input; // 
                        kdpRESIGNATIONDATE.TextBoxReadOnly = SCSHRConfiguration.IS_PICKER_READONLY.ToUpper() == "Y";
                        kdpRESIGNDATE.TextBoxReadOnly = SCSHRConfiguration.IS_PICKER_READONLY.ToUpper() == "Y";
                        break;
                }
    ...
    private DataTable getRESIGNVEType(string progId, string selectFields)
    {
        ...
        if (ex != null)
            DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"UC_KYTI_SCSHR_RESIGNVE.getRESIGNVEType.service.Error:{0}", ex.Message));
    ...
    /// <param name="e"></param>
    protected void kdpRESIGNDATE_TextChanged(object sender, EventArgs e)
    {
        KYTDatePicker _kdpRESIGNDATE = (KYTDatePicker)sender;
        kdpRESIGNATIONDATE.Text = _kdpRESIGNDATE.Text; // 自動填入實際離職日
    }
}
