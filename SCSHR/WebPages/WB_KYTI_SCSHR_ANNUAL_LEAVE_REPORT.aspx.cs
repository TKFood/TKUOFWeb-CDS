...
* 修改位置：
    * 「btnSearch_Click」中，查詢〈WATT0022400〉的結果註解
* **/
...
/// 個人特休查詢
/// </summary>
public partial class WB_KYTI_SCSHR_ANNUAL_LEAVE_REPORT : BasePage
{
    ...
    protected void gvItems_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            string value = ((HiddenField)gr.FindControl("hidSPECIALYEAR")).Value;
            string result = value;
            if (value == "0") result = "期初";
            else if (value == "1") result = "遞延";
            ((Label)gr.FindControl("lblSPECIALYEAR")).Text = result;
        }
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        "WATT0022400",
        "GetSpecLeaveData_Web",
        SCSHR.Types.SCSParameter.getPatameters(new
        {
            SpecialYear = txtYear.Text,
            LeaveEmp = ddlLeaveEmp.SelectedValue,
            EmployeeID = accounts
        }),
        out ex);
        //JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_ANNUAL_LEAVE_REPORT.btnSearch_Click.BOExecFunc::{0}::Send::{1}", "WATT0022400", txtYear.Text + " , " + ddlLeaveEmp.SelectedValue + " , " + accounts));
        //JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_ANNUAL_LEAVE_REPORT.btnSearch_Click.BOExecFunc::{0}::Result::{1}", "WATT0022400", Newtonsoft.Json.JsonConvert.SerializeObject(parameters)));

        if (ex != null)
            JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_ANNUAL_LEAVE_REPORT.btnSearch_Click.BOExecFunc.ERROR:{0}", ex.Message));
        if (parameters != null &&
            parameters.Length > 0)
        {
        ...
        if (dtResult != null)
        {
            ...
            dv.Sort = "TMP_EMPLOYEEID, SPECIALYEAR";
    ...
    private DataTable CreateGvitem()
    {
        ...
        dt.Columns.Add(new DataColumn("SPECIALYEAR", typeof(String))); // 特休年度
        dt.Columns.Add(new DataColumn("SENIORITY", typeof(String))); // 特休年資
        dt.Columns.Add(new DataColumn("YEARSPECIALDAYS", typeof(String))); // 年度預計可休天數
        dt.Columns.Add(new DataColumn("YEARSPECIALHOURS", typeof(String))); // 年度預計可休時數
        dt.Columns.Add(new DataColumn("CHANGEHOURS", typeof(String))); // 增減可休時數
        ...
        dt.Columns.Add(new DataColumn("TRANSPECHOURS", typeof(String))); // 折抵時數 
        dt.Columns.Add(new DataColumn("STARTDATE", typeof(String))); // 生效起始日期 
        dt.Columns.Add(new DataColumn("ENDDATE", typeof(String))); // 生效結束日期 
    ...
    protected void btnDLExcel_Click(object sender, EventArgs e)
    {
        ...
        {
            WebUtils.GridViewExportToExcel(stream, "個人特休查詢", gvItems, "微軟正黑體", "A1");
