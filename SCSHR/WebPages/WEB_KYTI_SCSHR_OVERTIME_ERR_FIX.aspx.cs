...
public partial class CDS_SCSHR_WebPages_WEB_KYTI_SCSHR_OVERTIME_ERR_FIX : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Field_ID = "KYTI_SCSHR_OVERTIME";
        if (!Page.IsPostBack)
        {
            ConstructorCommonSettings.setCommonSettings();
            AddSiteMapNode("加班單異常處理", Request.Url.AbsoluteUri);
        }
    }
    /// <summary>
    /// 過濾屬於表單的單號
    ...
    private DataTable filterBelongToForm(DataTable dtSource, string fieldID)
    {
        ...
        foreach (DataRow dr in dtSource.Rows)
        {
            ...
            using (DataSet ds = new DataSet())
            {
                ...
                {
                    DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"WEB_KYTI_SCSHR_OVERTIME_ERR_FIX.filterBelongToForm.TB_WKF_TASK.SELECT.ERROR:{0}", ex.Message));
                }
            }
        }
        return dtReturn;
    ...
    protected void gvOVH_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
    {
        ...
        if (gr.RowType == DataControlRowType.DataRow)
        {
            ...
            Label lblCHANGETYPE = gr.FindControl("lblCHANGETYPE") as Label;
            Label lblOT_START = gr.FindControl("lblOT_START") as Label;
            Label lblOT_END = gr.FindControl("lblOT_END") as Label;
            ...
            using (DataSet ds = new DataSet())
            {
                ...
                {
                    DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"WEB_KYTI_SCSHR_OVERTIME_ERR_FIX.gvOVH_RowDataBound.TB_WKF_TASK.SELECT.ERROR:{0}", ex.Message));
                }
            }
            if (isRightField)
            {
                ...
                lblCHANGETYPE.Text = ReBuildSCSHRForm.FindDropDownListText(dict.GetDataTable("krblCHANGETYPE"), dict.GetString("krblCHANGETYPE").ToString());
                lblOT_START.Text = (string)dict.GetString("kdtpOT_START");
                lblOT_END.Text = (string)dict.GetString("kdtpOT_END");
            }
        }
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(txtDOC_NBR.Text.Trim()))
        {
            ...
            using (DataSet ds = new DataSet())
            {
                ...
                {
                    sda.Fill(ds);
                    ViewState["gvOVH"] = filterBelongToForm(ds.Tables[0], Field_ID);
                    gvOVH.DataSource =(DataTable)ViewState["gvOVH"];
                    gvOVH.DataBind();
                }
                catch (Exception ex)
                {
                    DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"WEB_KYTI_SCSHR_OVERTIME_ERR_FIX.btnSearch_Click.TB_WKF_TASK.ERROR:{0}", ex.Message));
                }
            }
        }
    ...
    protected void btnReBuild_Click(object sender, EventArgs e)
    {
        if (gvOVH.Rows.Count > 0)
        {
            foreach (GridViewRow gr in gvOVH.Rows)
            {
                ...
                {
                    // 重新呼叫
                    //string RECALC_STATUS = ReBuildSCSHRForm.ReCalcWSAndReplaceOVHForm(lblDOC_NBR.Text);
                    ...
                    //lblMSG.Text = lblZTABLE_STATUS.Text != "O" ? RECALC_STATUS + "<br />" : "";
