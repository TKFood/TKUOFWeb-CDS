...
public partial class CDS_SCSHR_WebPages_WEB_KYTI_SCSHR_LEAVE_ERR_FIX : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Field_ID = "KYTI_SCSHR_LEAVE";
        if (!Page.IsPostBack)
        {
            ConstructorCommonSettings.setCommonSettings();
            AddSiteMapNode("請假單異常處理", Request.Url.AbsoluteUri);
        }
    }
    ...
    private DataTable filterBelongToForm(DataTable dtSource, string fieldID)
    {
        ...
        foreach (DataRow dr in dtSource.Rows)
        {
            ...
            using (DataSet ds = new DataSet())
            {
                ...
                {
                    DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"WEB_KYTI_SCSHR_LEAVE_ERR_FIX.filterBelongToForm.TB_WKF_TASK.SELECT.ERROR:{0}", ex.Message));
                }
            }
        }
        return dtReturn;
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(txtDOC_NBR.Text.Trim()))
        {
            ...
            using (DataSet ds = new DataSet())
            {
                ...
                {
                    sda.Fill(ds);
                    ViewState["gvLVH"] = filterBelongToForm(ds.Tables[0], Field_ID);
                    gvLVH.DataSource = (DataTable)ViewState["gvLVH"];
                    gvLVH.DataBind();
                }
                catch (Exception ex)
                {
                    DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"WEB_KYTI_SCSHR_LEAVE_ERR_FIX.btnSearch_Click.TB_WKF_TASK.ERROR:{0}", ex.Message));
                }
            }
        }
    ...
    {
        if (gvLVH.Rows.Count > 0)
        {
            foreach (GridViewRow gr in gvLVH.Rows)
            {
                Label lblZTABLE_STATUS = gr.FindControl("lblZTABLE_STATUS") as Label;
                Label lblWS_STATUS = gr.FindControl("lblWS_STATUS") as Label;
                Label lblDOC_NBR = gr.FindControl("lblDOC_NBR") as Label;
