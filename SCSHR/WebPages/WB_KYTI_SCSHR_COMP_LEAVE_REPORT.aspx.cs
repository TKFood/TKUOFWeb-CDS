...
* 修改位置：
    * 「btnSearch_Click」中，查詢〈ATT0023400〉的結果註解
* **/
...
/// 個人補休查詢
/// </summary>
public partial class WB_KYTI_SCSHR_COMP_LEAVE_REPORT : BasePage
{
    ...
    protected void gvItems_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (gr.RowType == DataControlRowType.DataRow)
        {
            ...
            switch (((HiddenField)gr.FindControl("hidSOURCETYPE")).Value)
            {
                case "0": ((Label)gr.FindControl("lblSOURCETYPE")).Text = "期初補休"; break;
                case "1": ((Label)gr.FindControl("lblSOURCETYPE")).Text = "加班單"; break;
                case "2": ((Label)gr.FindControl("lblSOURCETYPE")).Text = "遞延補休"; break;
            }
        }
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        SCSHR.Types.SCSParameter.getPatameters(new
        {
            ...
            ResidueHours = ddlResidueHours.SelectedValue,
            EmployeeID = accounts
        }),
        out ex);
        //JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_COMP_LEAVE_REPORT.btnSearch_Click.BOExecFunc::{0}::Send::{1}", "ATT0023400", ddlLeaveEmp.SelectedValue + " , " + ddlResidueHours.SelectedValue + " , " + accounts));
        //JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_COMP_LEAVE_REPORT.btnSearch_Click.BOExecFunc::{0}::Result::{1}", "ATT0023400", Newtonsoft.Json.JsonConvert.SerializeObject(parameters)));

        if (ex != null)
            JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_COMP_LEAVE_REPORT.btnSearch_Click.BOExecFunc.ERROR:{0}", ex.Message));
        if (parameters != null &&
            parameters.Length > 0)
        {
    ...
    private DataTable CreateGvitem()
    {
        ...
        dt.Columns.Add(new DataColumn("SOURCETYPE", typeof(String))); // 來源
        dt.Columns.Add(new DataColumn("SOURCEID", typeof(String))); // 單號
        dt.Columns.Add(new DataColumn("STARTDATE", typeof(String))); // 起始日期
        dt.Columns.Add(new DataColumn("ENDDATE", typeof(String))); // 結束日期
        dt.Columns.Add(new DataColumn("GHOURS", typeof(String))); // 平日時數
        dt.Columns.Add(new DataColumn("HHOURS", typeof(String))); // 休息日(H)時數
        dt.Columns.Add(new DataColumn("H2HOURS", typeof(String))); // 例假日(H2)時數 
        dt.Columns.Add(new DataColumn("H3HOURS", typeof(String))); // 變形休息日(H3)時數 
        dt.Columns.Add(new DataColumn("H4HOURS", typeof(String))); // 國定假日(H4)時數 
        dt.Columns.Add(new DataColumn("SHOURS", typeof(String))); // 特殊日時數
        dt.Columns.Add(new DataColumn("LEAVEHOURS", typeof(String))); // 已休時數 
        dt.Columns.Add(new DataColumn("CLOSEHOURS", typeof(String))); // 結清時數
        dt.Columns.Add(new DataColumn("RESIDUEHOURS", typeof(String))); // 剩餘時數
    ...
    protected void btnDLExcel_Click(object sender, EventArgs e)
    {
        ...
        {
            WebUtils.GridViewExportToExcel(stream, "個人補休查詢", gvItems, "微軟正黑體", "A1");
