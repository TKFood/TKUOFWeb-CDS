...
* 修改位置：
    * 「btnSearch_Click」中，查詢〈ATT0022500〉的結果註解
* **/
...
/// 個人彈休查詢
/// </summary>
public partial class WB_KYTI_SCSHR_ELAS_LEAVE_REPORT : BasePage
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        "ATT0022500",
        "GetFlexLeaveData_Web",
        SCSHR.Types.SCSParameter.getPatameters(new
        {
            FlexYear = txtYear.Text,
            LeaveEmp = ddlLeaveEmp.SelectedValue,
            EmployeeID = accounts
        }),
        out ex);
        //JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_ELAS_LEAVE_REPORT.btnSearch_Click.BOExecFunc::{0}::Send::{1}", "ATT0022500", txtYear.Text + " , " + ddlLeaveEmp.SelectedValue + " , " + accounts));
        //JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_ELAS_LEAVE_REPORT.btnSearch_Click.BOExecFunc::{0}::Result::{1}", "ATT0022500", Newtonsoft.Json.JsonConvert.SerializeObject(parameters)));

        if (ex != null)
            JGlobalLibs.DebugLog.Log(string.Format(@"WB_KYTI_SCSHR_ELAS_LEAVE_REPORT.btnSearch_Click.BOExecFunc.ERROR:{0}", ex.Message));
        if (parameters != null &&
            parameters.Length > 0)
        {
    ...
    private DataTable CreateGvitem()
    {
        ...
        dt.Columns.Add(new DataColumn("STARTDATE", typeof(String))); // 到職日期
        dt.Columns.Add(new DataColumn("SEPARATIONDATE", typeof(String))); // 離職日期
        dt.Columns.Add(new DataColumn("FLEXDATE", typeof(String))); // 彈休日期
        dt.Columns.Add(new DataColumn("VACATIONNAME", typeof(String))); // 假別名稱
        dt.Columns.Add(new DataColumn("FLEXDAYS", typeof(String))); // 可休天數
        dt.Columns.Add(new DataColumn("FLEXHOURS", typeof(String))); // 可休時數
        dt.Columns.Add(new DataColumn("LEAVEHOURS", typeof(String))); // 已休時數
        dt.Columns.Add(new DataColumn("RESIDUEHOURS", typeof(String))); // 剩餘時數
        dt.Columns.Add(new DataColumn("FLEXSTARTDATE", typeof(String))); // 彈休可休開始日期
        dt.Columns.Add(new DataColumn("FLEXENDDATE", typeof(String))); // 彈休可休截止日期
    ...
    protected void btnDLExcel_Click(object sender, EventArgs e)
    {
        ...
        {
            WebUtils.GridViewExportToExcel(stream, "個人彈休查詢", gvItems, "微軟正黑體", "A1");
