...
<script>
    ...
    $(document).ready(function () {        
        ...
        if (status == "APPLY") {
            $("#<%=divContent.ClientID %>").append("<tr style='background-color:#0088A8'><th style='width:20%;color:#fff;text-align:center'>品號</th><th style='width:30%;color:#fff;text-align:center'>品名</th><th style='width:30%;color:#fff;text-align:center'>規格</th><th style='width:10%;color:#fff;text-align:center'>數量</th><th style='width:10%;color:#fff;text-align:center'>刪除</th></tr>");
        } else {
            $("#<%=divContent.ClientID %>").append("<tr style='background-color:#0088A8'><th style='width:20%;color:#fff;text-align:center'>品號</th><th style='width:30%;color:#fff;text-align:center'>品名</th><th style='width:30%;color:#fff;text-align:center'>規格</th><th style='width:10%;color:#fff;text-align:center'>數量</th><th style='width:10%;color:#fff;text-align:center'>總倉庫存</th></tr>");
        }
    ...
    function loadSourceData() {
        ...
        var status = $("#<%=hdStatus.ClientID %>").val();
        var item_no_list = "";
        var facname = $("#CobManufacturer").val();
        $.each(data, function (i, item) {
            var obj = { "no": item.no, "name": item.name, "spec": item.spec, "qty": item.qty,"Facname": facname};
            itemsarr.push(obj);
            var no = item.no;
            if (status == "APPLY") {
                $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'><td>" + item.no + "</td><td>" + item.name + "</td><td>" + item.spec + "</td><td><input type='number'onchange='javascript:updateqty(\"" + no + "\",this.value)' value='" + item.qty + "'></td>><td style='text-align:center'><button onclick='javascript:delitem(\"" + no + "\")' class='btn btn-danger btn-sm'>刪除</button></td></tr>");
            } else {
    ...
    function getStockCallback(response) {
        ...
        $.each(data, function (i, item) {
            ...
            if (status != "APPLY") {
                ...
                $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'><td>" + item.no + "</td><td>" + item.name + "</td><td>" + item.spec + "</td><td style='text-align:center'>" + item.qty + "</td><td style='text-align:center'>" + stockqty + "</td></tr>");               
            }
        });
    }
    ...
    $("#btnAdd").click(function () {
        ...
        if (checknoexist(no)) {
            ...
            $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'><td>" + no + "</td><td>" + itemname + "</td><td>" + spec + "</td><td><input type='number' value='" + qty + "' onchange='javascript:updateqty(\"" + no + "\",this.value)'></td><td style='text-align:center'><button onclick='javascript:delitem(\"" + no + "\")' class='btn btn-danger btn-sm'>刪除</button></td></tr>");
            $("#Qty").val("1");          
            var obj = { "no": no, "name": itemname, "spec": spec, "qty": qty };
            itemsarr.push(obj);
