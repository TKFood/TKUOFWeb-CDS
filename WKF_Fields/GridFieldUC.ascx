...
<script>
    ...
    $(document).ready(function () {        
        ...
        if (status == "APPLY") {
            $("#<%=divContent.ClientID %>").append("<tr style='background-color:#0088A8'>" +
                ...
                "<th style='width:10%;color:#fff;text-align:center'>新員</th>" +
                "<th style='width:10%;color:#fff;text-align:center'>以舊換新</th>" +
                "<th style='width:10%;color:#fff;text-align:center'>數量</th>" +
                "<th style='width:10%;color:#fff;text-align:center'>刪除</th></tr >");
        } else {
            $("#<%=divContent.ClientID %>").append("<tr style='background-color:#0088A8'>" +
                "<th style='width:20%;color:#fff;text-align:center'>品號</th>" +
                "<th style='width:20%;color:#fff;text-align:center'>品名</th>" +
                ...
                "<th style ='width:10%;color:#fff;text-align:center'>以舊換新</th>" +
                "<th style='width:10%;color:#fff;text-align:center'>數量</th>" +
                "<th style='width:10%;color:#fff;text-align:center'>總倉庫存</th></tr>");
        }
    ...
    function loadSourceData() {
        ...
        $.each(data, function (i, item) {           
            var obj = { "no": item.no, "name": item.name, "spec": item.spec, "qty": item.qty, "transnew": item.transnew, "chknew": item.chknew };
            itemsarr.push(obj);
            var no = item.no;
            //var chknew = item.chknew;
            ...
            if (status == "APPLY") {
                ...
                if (item.chknew == true) {
                    $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'>" +
                        ...
                        "<td>" + item.name + "</td>" +
                        "<td>" + item.spec + "</td>" +
                        "<td style='text-align:center'><input type='checkbox'onchange='javascript:updatetransnew(\"" + no + "\")' checked></td>" +             
                        "<td>" + chktransnew + "</td>" +
                    ...
                    $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'>" +
                        ...
                        "<td>" + item.name + "</td>" +
                        "<td>" + item.spec + "</td>" +
                        "<td style='text-align:center'><input type='checkbox'onchange='javascript:updatetransnew(\"" + no + "\")'></td>" +   
                        "<td>" + chktransnew + "</td>" +
                        "<td><input type='number'onchange='javascript:updateqty(\"" + no + "\",this.value)' value='" + item.qty + "'></td>" +
                        "<td style='text-align:center'><button onclick='javascript:delitem(\"" + no + "\")' class='btn btn-danger btn-sm'>刪除</button></td></tr>");
                   //$("#" + item.no).children('td').eq(3).find('input:checkbox').attr('checked', item.chknew);
                   //$("#" + item.no).children('td').eq(3).find('input[type="checkbox"]').attr({ disabled: "disabled" });
                }
    ...
    // 更新checkbox
    function updatetransnew(no) {    
        var idx = 0;
        itemsarr.forEach(function (obj) {
            if (obj.no == no) {
                obj.chknew = $("#" + no).children('td').eq(3).find('input[type="checkbox"]').prop('checked');
                itemsarr[idx] = obj;
            }
            idx++;
        });
    ...
    $("#btnAdd").click(function () {
        ...
        if (checknoexist(no)) {
            ...
            $("#Qty").val("1");
            //////////////////////////////
            var obj = { "no": no, "name": itemname, "spec": spec, "qty": qty, "transnew": transnew, "chknew": "false" };
            itemsarr.push(obj);
