...
<script>
    ...
    function getDeptApplyStockCallback(response) {    
        ...
        $.each(JSON.parse(response), function (i, item) {            
            ...
            //alert("item.no : " + item.no + " price : " + price + " qty: " + item.qty + " subtotal: " + subtotal);
            var obj = { "no": item.no, "name": item.name, "spec": item.spec, "qty": item.qty, "price": price, "subtotal": subtotal };
            itemsarr.push(obj);
    ...
    function getStockCallback(response) {
        ...
        $.each(data, function (i, item) {
            ...
            var obj = { "no": item.no, "name": item.name, "spec": item.spec, "qty": item.qty, "price": item.price, "subtotal": item.subtotal };
            itemsarr.push(obj);
        });
        //$("#talPrice").html("");
        //if ($("#talPrice").html() == "") {
    ...
    $("#btnAdd").click(function () {      
        ...
        if (checknoexist(no)) {
            ...
            $("#<%=divContent.ClientID %>").append("<tr id='" + no + "'>"
                ...
                + "</tr>");
            $("#Qty").val("1");         
            var obj = { "no": no, "name": itemname, "spec": spec, "qty": qty, "price": price, "subtotal": subtotal};
            itemsarr.push(obj);
            $("#<%=hdContent.ClientID %>").val(JSON.stringify(itemsarr));
        }
       // $("#talPrice").html("");
